...
class UnixDaemon(object):
    ...
    def start(self):
        pid = self._getpid()
        if pid:
            sys.stderr.write("%s already running with pid %i\n" % pid)
            sys.exit(1)
    ...
    def status(self):
        pid = self._getpid()
        if pid:
            sys.stdout.write("%s is running with pid %i\n" % (self.name, pid))
        else:
...
def add_optparse_daemon_options(parser):
    ...
    start_stop_group.add_option('-p', '--pidfile', dest='pidfile', metavar='FILE', default=os.path.join(sys.path[0], 'ingraph-cleanup.pid'),
