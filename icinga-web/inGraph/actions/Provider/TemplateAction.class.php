...
class inGraph_Provider_TemplateAction extends inGraphBaseAction
{
    public function executeWrite(AgaviRequestDataHolder $rd)
    {
        ...
        try {
            $plots  = $this->getBackend()->fetchPlots($host, $service, $rd->getParameter('parentService', null));
         ...
         * if (!$plots['total']) { ... }
         */
        $manager = new inGraph_Template_Manager(AgaviConfig::get('modules.ingraph.templates'));
        $manager->collectTemplates();
        $template           = $manager->fetchTemplate($service);
        $contentAndQuery    = $template->compile($plots['plots']);
        $this->setAttribute(
            'template',
            array(
                'name'      => $template->getSplFileInfo()->getBasename(),
                'content'   => $contentAndQuery->content,
                'isDefault' => $manager->isDefault($template),
                'query'     => $contentAndQuery->query
            )
        );
