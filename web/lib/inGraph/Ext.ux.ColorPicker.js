...
Ext.ux.ColorPicker = Ext.extend( Ext.BoxComponent, {
	...
	, setColor: function(c) {
		...
		Ext.get( this.cpGetId( 'cOriginal' ) ).setStyle({
			'background': '#' + c
		});
	...
	, resetColor: function( event, element ) {
		Ext.getCmp( this.cpGetId( 'iHexa' ) ).setValue(this.originalColor);
		this.updateFromIHexa();
	}
	/**
...
//Ext.extend(Ext.ux.menu.ColorItem, Ext.menu.Adapter, { //+X+REMOVED
Ext.extend(Ext.ux.menu.ColorItem, { //+X+ADDED
	// private
